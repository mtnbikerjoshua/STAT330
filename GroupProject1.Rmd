...
```{r}
#MAE
sum(abs(premier$residuals)) / premier_lm$df.residual

```

```{r}
#R2 AR2 hypothesis slope test
summary(premier_lm)
```

r2 = .6983
ar2 = .6956
hypothesis slope test: It is significant because the slope isn't 0, and the
p-value is less than .05, so we reject the null hypothesis that the slope is
insignificant.
