...
The outcome variable is the wind speed at the canidate site.
...
```{r, fig.align='center'}
ggplot(data = wind, mapping = aes(x = RSpd, y = CSpd)) +
  geom_point() +
...
ggplot(data = wind, mapping = aes(x = RSpd, y = CSpd)) +
  geom_point() +
  geom_abline(slope = model$coefficients["RSpd"],
...
$\text{CanidateSpeed}_i=3.1412+0.7557\cdot\text{RefSpeed}_i+\epsilon_i$
...
Our regression model did not go that high and we have no idea if the model is accuarte beyond the data that we used.
