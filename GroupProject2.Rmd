...
Next, we will run an initial linear model and take a look at the residuals to see what transformations might need to be done. We will also refer back to the scatterplots above.
```{r}
...
sq_model <- lm(Life.Expectancy ~ ., data = life_expect_sq)
...
model_15 <- lm(Life.Expectancy ~ ., data = life_expect_15)
...
# Sqrt transformation
life_expect_sqrt <- life_expect %>%
  mutate(Life.Expectancy = sqrt(Life.Expectancy))
sqrt_model <- lm(Life.Expectancy ~ ., data = life_expect_sqrt)
resid_vs_fitted(sqrt_model)
jcreg_hist(sqrt_model)
jcreg_qq(sqrt_model)
shapiro.test(sqrt_model$residuals)

# Log transformation
life_expect_log <- life_expect %>%
  mutate(Life.Expectancy = log(Life.Expectancy))
log_model <- lm(Life.Expectancy ~ ., data = life_expect_log)
resid_vs_fitted(log_model)
jcreg_hist(log_model)
jcreg_qq(log_model)
...
cu_model <- lm(Life.Expectancy ~ ., data = life_expect_cu)
resid_vs_fitted(cu_model)
jcreg_hist(cu_model)
jcreg_qq(cu_model)
shapiro.test(cu_model$residuals)

# Power of 4 transformation
life_expect_4 <- life_expect %>%
  mutate(Life.Expectancy = Life.Expectancy^4)
model_4 <- lm(Life.Expectancy ~ ., data = life_expect_4)
resid_vs_fitted(model_4)
jcreg_hist(model_4)
jcreg_qq(model_4)
shapiro.test(model_4$residuals)
```
Based on the transformations attempted, the best one appears to be a cubic transformation. The cubic transformation sees great improvement in homoscedasticity and some improvement in normality. We will complete the rest of the analysis on a cubic scale.

We will also check the need for transformations of the predictor variables.
```{r}
life_expect_cu_cont <- life_expect_cu %>%
  select(-Status)
map(2:length(life_expect_cu_cont), scatter,
    data = life_expect_cu_cont, y_ind = 1)
```
